...
class RNNCell(object):
    ...
    def forward(self, x, h_prev_t):
        ...
        """
        z_h = h_prev_t @ self.W_hh.T + self.b_hh
        z_x = x @ self.W_ih.T + self.b_ih
        h_t = self.activation.forward(z_h + z_x)
        return h_t
    ...
    def backward(self, delta, h_t, h_prev_l, h_prev_t):
        ...
        N = h_prev_l.shape[0]
        self.dW_ih += dz.T @ h_prev_l / N
        self.dW_hh += dz.T @ h_prev_t / N
        self.db_ih += np.einsum('nc->c', dz, optimize='optimal') / N
        self.db_hh += np.einsum('nc->c', dz, optimize='optimal') / N
