...
class Upsample2d():
    ...
    def forward(self, A):
        ...
        """
        B, C_in, H_in, W_in = A.shape
        H_out = self.upsampling_factor*(H_in-1)+1
        W_out = self.upsampling_factor*(W_in-1)+1
        Z = np.zeros((B, C_in, H_out, W_out))
...
class Downsample2d():
    ...
    def forward(self, A):
        ...
        Return:
            Z (np.array): (batch_size, in_channels, output_height, output_width)
        """
        self.B, self.C_in, self.H_in, self.W_in = A.shape
    ...
    def backward(self, dLdZ):
        ...
        Return:
            dLdA (np.array): (batch_size, in_channels, input_height, input_width)
        """
        dLdA = np.zeros((self.B, self.C_in, self.H_in, self.W_in))
